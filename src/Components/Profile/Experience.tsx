import { ActionIcon, Timeline } from "@mantine/core";
import { IconBriefcase, IconPencil, IconPlus, IconX } from "@tabler/icons-react";
...
const Experience=()=>{
    ...
    <div className="flex flex-col gap-8">
        <Timeline active={profile?.experiences?.length} bulletSize={24} lineWidth={2} color="brightSun.4">
            {profile?.experiences?.map((exp:any, index:number) => (
                <Timeline.Item key={index} bullet={<IconBriefcase size={12} />} className="!pt-1">
                    <ExpCard edit={edit} index={index} {...exp} />
                </Timeline.Item>
            ))}
        </Timeline>
