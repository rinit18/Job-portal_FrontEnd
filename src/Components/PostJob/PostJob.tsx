...
import { Button, NumberInput, TagsInput, Textarea, Tooltip } from "@mantine/core";
                ...
                <div className="flex justify-between items-center mb-1">
                    ...
                    <Tooltip label="AI will generate a description based on Job Title and About fields" withArrow position="left">
                        <Button
                            ...
                            loading={aiLoading}
                            onClick={async () => {
                                const title = form.getValues().jobTitle;
                                const notes = form.getValues().about;
                                if (!title) { errorNotification("Missing Info", "Please enter a Job Title first."); return; }
                                setAiLoading(true);
                                try {
                                    const desc = await generateJobDescription(title, notes || "");
                                    setEditorData(desc);
                                    form.setFieldValue('description', desc);
                                    successNotification("AI Generated", "Job description filled in!");
                                } catch {
                                    errorNotification("AI Error", "Could not generate description. Check your Gemini API key.");
                                } finally {
                                    setAiLoading(false);
                                }
                            }}
